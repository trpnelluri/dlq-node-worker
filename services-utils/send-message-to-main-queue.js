...
async function sendMsgToMainQueue (message, transId, sourceQueueURL, sendMsgToSecDLQ) {
    ...
    return new Promise((resolve, reject) => {
        ...
        logger.info(`sendMsgToMainQueue, nbReplay: ${nbReplay} New messageDeduplicationId: ${messageDeduplicationId} sendMsgToSecDLQ ${sendMsgToSecDLQ}`)
        if ( sendMsgToSecDLQ ) {
        ...
        logger.debug(`sendMsgToMainQueue, targetQueueQRL: ${targetQueueQRL} nbReplay After Update: ${nbReplay}`)
        const sendMsgParams = {
            MessageBody: msgBody,
        ...
        logger.error(`sendMsgToMainQueue, ERROR: ${error}` )
    });
}
